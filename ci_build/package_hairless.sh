...
PACKAGE_DIR="hairless-midiserial-${VERSION}"
...
if [ "$TARGET" == "win32" ]; then
    zip -r "hairless-midiserial-${TARGET}-${VERSION}.zip" "${PACKAGE_DIR}"
else
    tar -zcf "hairless-midiserial-${TARGET}-${VERSION}.tgz" "${PACKAGE_DIR}"
fi
